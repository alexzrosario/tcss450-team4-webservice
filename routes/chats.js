...
/**
 * @api {get} /chats Request to get chat ids
 * @apiName GetChats
 * @apiGroup Chats
 * 
 * @apiDescription Get all the chat ids that the user is in.
 * 
 * @apiHeader {String} authorization Valid JSON Web Token JWT
 * 
 * @apiSuccess {Number} rowCount the number of chat ids the user is in
 * @apiSuccess {Object[]} chatId List of chatId the user is in.
 * 
...
router.get("/", (request, response, next) => {
    ...
    // Perform the SELECT
    let query = `SELECT ChatMembers.ChatId, Chats.Name
                FROM ChatMembers
 ...
 * @apiGroup Chats
 * 
 * @apiDescription Adds the user associated with their email.
 * 
 * @apiHeader {String} authorization Valid JSON Web Token JWT
 * 
 * @apiParam {Number} chatId the chat to add the user to
 * @apiParam {String} email the email of the user to add to the chat
 * 
...
router.put("/:chatId/:email", (request, response, next) => {
    ...
    pool.query(query, values)
        .then(result => {
            if (result.rowCount == 0) {
                ...
                console.log("Before change Param Email: " + request.params.email)
                request.params.email = result.rows[0].memberid
                console.log("Param Email: " + request.params.email)
                next()
            }
        }).catch(error => {
            response.status(400).send({
    ...
    // Insert the memberId from the chat
